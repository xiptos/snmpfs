...
public class FakeFilesystem implements Filesystem3, XattrSupport {
	...
	private static class N {
		...
		public String toString() {
			...
			return cn.substring(cn.indexOf("$")) + "[ name=" + name + ", mode=" + Integer.toOctalString(mode)
					+ "(OCT) ]";
		}
	}
	...
	public int getattr(String path, FuseGetattrSetter getattrSetter) throws FuseException {
		...
		if (n instanceof D) {
			D d = (D) n;
			getattrSetter.set(d.hashCode(), FuseFtypeConstants.TYPE_DIR | d.mode, 1, 0, 0, 0, d.files.size()
					* NAME_LENGTH, (d.files.size() * NAME_LENGTH + BLOCK_SIZE - 1) / BLOCK_SIZE, time, time, time);

			return 0;
		} else if (n instanceof F) {
			F f = (F) n;
			getattrSetter.set(f.hashCode(), FuseFtypeConstants.TYPE_FILE | f.mode, 1, 0, 0, 0, f.content.length,
					(f.content.length + BLOCK_SIZE - 1) / BLOCK_SIZE, time, time, time);

			return 0;
		} else if (n instanceof L) {
			L l = (L) n;
			getattrSetter.set(l.hashCode(), FuseFtypeConstants.TYPE_SYMLINK | l.mode, 1, 0, 0, 0, l.link.length(),
					(l.link.length() + BLOCK_SIZE - 1) / BLOCK_SIZE, time, time, time);
	...
	public int getdir(String path, FuseDirFiller filler) throws FuseException {
		...
		if (n instanceof D) {
			for (N child : ((D) n).files.values()) {
				int ftype = (child instanceof D) ? FuseFtypeConstants.TYPE_DIR
						: ((child instanceof F) ? FuseFtypeConstants.TYPE_FILE
								: ((child instanceof L) ? FuseFtypeConstants.TYPE_SYMLINK : 0));
				if (ftype > 0)
